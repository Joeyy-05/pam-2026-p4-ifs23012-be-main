...
fun Application.configureRouting() {
    ...
    val headsetService: HeadsetService by inject()
    ...
    routing {
        ...
        // --- Route Headsets ---
        route("/headsets") {
            get { headsetService.getAllHeadsets(call) }
            post { headsetService.createHeadset(call) }
            get("/{id}") { headsetService.getHeadsetById(call) }
            put("/{id}") { headsetService.updateHeadset(call) }
            delete("/{id}") { headsetService.deleteHeadset(call) }
            get("/{id}/image") { headsetService.getHeadsetImage(call) }
        }
